...
class Service extends _adapterCommons.AdapterService {
  ...
  _createQuery(params = {}) {
    ...
    if (joinOnSameTable) {
      query.alias(`_${this.Model.tableName}`);
    }
  ...
  _find(params = {}) {
    const find = (params, count, filters, query) => {
      ...
      if (count) {
        ...
        const countMethod = query.$joinRelation || query.$joinEager || countColumns.length > 1 ? "countDistinct" : "count";
        return Promise.all([this.Model.query()[countMethod]({
          total: countColumns
        }).from(qb => {
          builder.limit(this.options.maxTotalLimit || 10000).toKnexQuery(qb).as('temp');
        }), executeQuery(0)]).then(results => ({ ...results[1],
          total: parseInt(results[0][0].total, 10)
